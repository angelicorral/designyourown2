...
(function (h) {
    var n = function (d, n) {
        ...
        c = m.children(".fpd-toolbar").append('<div class="fpd-color-picker fpd-clearfix"><input type="text" id="color1" value="" disabled="disabled" />&nbsp;<input type="text" id="color2" value="" disabled="disabled" />&nbsp;<input type="text" id="color3" value="" disabled="disabled" /></div><select class="fpd-fonts-dropdown"></select><input type="text" value="" class="fpd-text-input" /><div class="fpd-z-switcher ui-state-default" title="' + f.labels.zPositionSwitcher + '"><span class="ui-icon ui-icon-carat-2-n-s"></span></div><div class="fpd-rotator ui-state-default" title="Rotate Element"><span class="ui-icon ui-icon-arrowrefresh-1-e"></span></div><div class="fpd-removebutton ui-state-default" title="Remove Element"><span class="ui-icon ui-icon-trash"></span></div><div class="fpd-deselect ui-state-default" title="' + f.labels.deselectButton + '"><span class="ui-icon ui-icon-closethick"></span></div>');
        /*END*/
        ...
        this.addElement = function (j, u, c, a, r) {
            ...
            }).attr("title", c).data("source", u).attr("id", r.children(".fpd-element").size() - 1);
            
            a.z = r.children(".fpd-element").size() - 1;
            da(d, a.degree);
            
            //this is where a.colors is parsed as array or string and where current color is determined
            if (a.colors && "string" == typeof a.colors)
                if (0 == a.colors.indexOf("#")) {
                	c = a.colors.replace(/\s+/g, "").split(","), 
                	a.colors = c, 
                	a.currentColor = c[0];
                }
                else {
                    c = s[0];
                    for (r = 0; r < c.elements.length; r++) 
                    	c.elements[r].title, c.elements[r].title == a.colors && 
                    	(a.currentColor = c.elements[r].parameters.currentColor ? c.elements[r].parameters.currentColor : c.elements[r].parameters.colors)
                }
             
             /*BEGIN - ADDED BY ACORRAL*/   
             if (a.designColors && "string" == typeof a.designColors)
                if (0 == a.designColors.indexOf("#")) v = a.designColors.replace(/\s+/g, "").split(","), a.designColors = v;
                
             if (a.paletteColors && "string" == typeof a.paletteColors)
                if (0 == a.paletteColors.indexOf("#")) v = a.paletteColors.replace(/\s+/g, "").split(","), a.paletteColors = v;
            ...
            if ("image" == j) {
                ...
                e.onload = function () {
                    ...
                    if (0 < a.colors.length) {
                        ...
                        d.append(r);
                        if(typeof a.designColors != "undefined")
	                        for(var i=0; i<a.designColors.length; i++){
	            	            oldColor = a.colors[i];
	                        	fa(g, a.designColors[i]);
	                        }
	                    else{
        ...
        var sa = function (j, overrideDesign) {
        	//alert("parse");
        	var product = l.getProduct(true);
            ...
            t.css({height: m.height() * j.length,top: 0});
            ...
            for (var a = 0; a < j.length; ++a) {
                ...
                ba(c.title, c.elements, false);
                if(overrideDesign && typeof product[a] != 'undefined'){
                	ba(c.title, product[a].elements, true);
                }
                1 < j.length && u.append('<li><img src="' + c.thumbnail + '" title="' + c.title + '" /></li>')
            }
            ...
            V = t.children("div:first")
        }, in_array = function in_array(array, id) {
			    ...
			    return false;
			},ba = function (j, a, hidebases) {
                for (var c = 0; c < a.length; ++c) {
                    var b = a[c];
                    var titlesplit = b.title.split(".");
                ...
                j.addClass("fpd-editable").children("img, canvas, p").css("cursor", "pointer").click(function () {
                    ...
                    /*BEGIN ADDED BY ACORRAL*/
                    if(u.removable)
                    	c.children(".fpd-removebutton").show();
                    else
                    	c.children(".fpd-removebutton").hide();
                    	
                    if(u.rotatable)
                    	c.children(".fpd-rotator").show();
                    else
                    	c.children(".fpd-rotator").hide();
                    /*END ADDED BY ACORRAL*/
                   ...
                   /*Array.isArray(u.colors) ? 
                   ( ha.children("input").val(u.currentColor), 1 < u.colors.length ? ha.children("input").spectrum("destroy").spectrum({
                        preferredFormat: "hex",
                        showPaletteOnly: !0,
                   ...
                   if(Array.isArray(u.designColors)){
                   		var palette = colorPalette;
                   		if(typeof u.paletteColors != 'undefined')
                   			palette = u.paletteColors;
                   			
	                   	for (var i=0; i<3; i++)
	                   		{
	                   			el = ha.find("#color"+(i+1));
	                   			var elementid = "color"+(i+1);
	                   			if(i<u.designColors.length){
	                   			el.val(u.designColors[i]);
                ...
                if(typeof j.data("params").designColors != 'undefined'){
	                //set the designColor changed to the new selected color
	                var index = j.data("params").designColors.indexOf(oldColor);
	                
	                //if oldColor is not found, just set the designColors [0] index
	                if(typeof j.data("params").designColors[index] == 'undefined')
	                	j.data("params").designColors[0] = a;
	                else
	                	j.data("params").designColors[index] = a;
	                
	                //ha.children("input").spectrum("set", a);
	                ha.find("#color"+(index+1)).spectrum("set", a);	
                }
                
                var b = j.attr("title"),
                    r = a;
                    
                //this code snippet sets the current color of all views of the product to the chosen color
            ...
            /*BEGIN ADDED BY ACORRAL*/
            isApprox = function isApprox(a, b, range) {
  				var d = a - b;
			...
			fa = function (j, a) {
				...
				/*BEGIN EDITED BY ACORRAL*/
            	//this is where canvas image coloring happens
            		var newRed =parseInt(W(a).substring(0, 2), 16);
            		var newGreen =parseInt(W(a).substring(2, 4), 16);
            		var newBlue =parseInt(W(a).substring(4, 6), 16);   
            		
            		//alert("oldcolor: "+oldColor)
            		if(typeof oldColor == 'undefined'){
            			for (var c = j.getImageData(0, 0, j.canvas.width, j.canvas.height), r = c.data, b = 0; b < r.length; b += 4){ 
                		r[b] = newRed, r[b + 1] = newGreen, r[b + 2] = newBlue;
                		}
            		}
            		else{
            			// if color is #fff or #000, fix it to proper hex
	            		if(oldColor == "#fff")
	            			oldColor = "#ffffff";
	            		if(oldColor == "#000")
	            			oldColor = "#000000";
	            			
	            		 var oldRed =parseInt(W(oldColor).substring(0, 2), 16);
	            		 var oldGreen =parseInt(W(oldColor).substring(2, 4), 16);
	            		 var oldBlue =parseInt(W(oldColor).substring(4, 6), 16);
	            		 //alert(oldRed + " "+ oldGreen + " "+ oldBlue);             		
			
		                for (var c = j.getImageData(0, 0, j.canvas.width, j.canvas.height), r = c.data, b = 0; b < r.length; b += 4){
		                	//hex color to rgb convert with Approximation
		                	//if (r[b] == oldRed && r[b + 1] == oldGreen && r[b + 2] == oldBlue)
		                	if (isApprox(r[b], oldRed, 120) && isApprox(r[b+1], oldGreen, 120) && isApprox(r[b+2], oldBlue, 90)) 
		                		r[b] = newRed, r[b + 1] = newGreen, r[b + 2] = newBlue;
		                }	
            		}
        ...
        g.click(function () {
            ...
            q.css("z-index", ++j);
            if(V.children("div").size()<j)
            	q.css("z-index", V.children("div").size());
            //V.children("div").size() == j && q.css("z-index", -1);
            q.data("params").z = parseInt(q.css("z-index"))
        });
        //this is where custom text creation happens
        ...
        c.children(".fpd-reset").click(function () {
            if (q) {
                ...
                if(Array.isArray(a.designColors)){
	                   	for (var i=0; i<3; i++)
	                   		{
	                   			el = ha.find("#color"+(i+1));
	                   			if(i<a.designColors.length){
	                   			el.val(a.designColors[i]);
	                   			el.spectrum("destroy").spectrum({
		                        preferredFormat: "hex",
		                        showPaletteOnly: !0,
		                        palette: colorPalette,
        ...
        c.children(".fpd-removebutton").click(function () {
            0 != q.data("params").price && (F -= q.data("params").price, b.trigger("priceChange", [q.data("params").price, F]));
                        q.remove();
                        $();
                        return !1
        });
        c.children(".fpd-rotator").click(function () {
        	var degree= q.data("params").degree;
        	if(degree<360)
        		degree=degree+10;
        	else
        		degree=0;
            q.css("-moz-transform", "rotate(" + degree + "deg)"), q.css("-webkit-transform", "rotate(" + degree + "deg)"), q.css("-o-transform", "rotate(" + degree + "deg)"), q.css("-ms-transform", "rotate(" + degree + "deg)");
            q.data("params").degree = degree;
        });
        /*END ADDED BY ACORRAL for other tools*/
        
        /* for further review (resize up and down buttons)
        c.children(".fpd-resizeup").click(function () {
        	//resizeup logic here
        	a = q.data("params");
        	
        	if (0 < q.children("img, canvas").size()) {
        		var wdth = a.width + 5;
        		wdth > f.maxImageWidth && (wdth = f.maxImageWidth);
                r = Math.round(a.originHeight * (wdth / a.originWidth));
                if (r > f.maxImageHeight || r < f.minImageHeight) return !1;
                        q.width(wdth).height(r).children("img, canvas").css({
                            width: wdth,
                            height: r
                        });
                        q.data("params").width = wdth;
                        q.data("params").height = r;
                        
                        f.editorMode && ea()
            }
        });
        c.children(".fpd-resizedown").click(function () {
        	//resizedown logic here
        });
        */
    ...
    h.fn.fancyProductDesigner.defaults = {
        ...
        customTextParamters: {designColors: ["#0000000"]},
        ...
        labels: {
            ...
            zPositionSwitcher: "Bring to Front",
            resetButton: "Reset",
